...
> [!Tip] we use usually use GetDefault() for most peripherals. For example, there is only one GPIO controller on most systems. This is not the case with PWM. Never use the Default controller and  always> select the proper channel on the correspondent controller.

> [!Tip] PWM2.3 is channel 3 on controller 2
...
> [!Tip] most servos will have a 1.25ms min and 1.75max.
